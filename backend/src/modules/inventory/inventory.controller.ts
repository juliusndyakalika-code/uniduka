...
// GET /inventory/audit
// Returns every product with >1 inventory row, showing:
//   rowCount      — how many InventoryItem rows exist
//   currentStock  — what the rows currently sum to (may be wrong due to bug)
//   correctStock  — sum of all StockMovements (the true figure)
//   discrepancy   — difference (correctStock − currentStock)
export async function inventoryAudit(req: AuthRequest, res: Response) {
  const sid = shop(req);

  // All inventory rows grouped by product
  const rows = await prisma.inventoryItem.findMany({
    where: { shopId: sid },
    include: { product: { select: { name: true, sku: true, unit: true, isActive: true } } },
  });

  // Group by productId
  const byProduct = new Map<string, { name: string; sku: string; unit: string; isActive: boolean; rowCount: number; currentStock: number }>();
  for (const r of rows) {
  ...
  // Only care about products with multiple rows
  const affected = [...byProduct.entries()].filter(([, v]) => v.rowCount > 1);
  if (affected.length === 0) return R.ok(res, []);

  // For each affected product, sum its StockMovements to get the true stock
  const result = await Promise.all(affected.map(async ([productId, info]) => {
    const agg = await prisma.stockMovement.aggregate({
      where: { shopId: sid, productId },
      _sum: { quantity: true },
    });
    const correctStock = agg._sum.quantity ?? 0;
    return {
      productId,
      name: info.name,
      sku: info.sku,
      ...
      correctStock,
      discrepancy: correctStock - info.currentStock,
    };
  }));

  // Sort by biggest discrepancy first
  result.sort((a, b) => Math.abs(b.discrepancy) - Math.abs(a.discrepancy));
