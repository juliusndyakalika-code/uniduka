...
function requireShop(req: AuthRequest, res: Response): string | null {
  const s = shop(req);
  if (!s) { R.badRequest(res, 'No shop context — please select a shop and log in again'); return null; }
  return s;
}
...
export async function getStatus(req: AuthRequest, res: Response) {
  ...
  const open = await prisma.timeClock.findFirst({
    where: { shopId, userId: uid(req), clockOut: null },
...
export async function clockIn(req: AuthRequest, res: Response) {
  ...
  const entry = await prisma.timeClock.create({
    data: { shopId, userId: uid(req), clockIn: new Date(), note: req.body.note },
  });
...
export async function deleteShift(req: AuthRequest, res: Response) {
  const shopId = requireShop(req, res);
  if (!shopId) return;
  const shift = await prisma.timeClock.findFirst({ where: { id: req.params.id, shopId } });
